...
export interface LdskcfMemberFormForm {
  address: string;
  city: string;
  const { 
      memberidno: string;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const filePath = path.join(publicDir, 'ldskcf_member_details.xlsx');
    let workbook;
    let ws1, ws2;
    ...
    const fileUrl = `https://www.birdviewmicroinsurance.com/ldskcf_member_details.xlsx`;
    ...
    const mailOptions = {
      ...
      attachments: [
        {
          filename: 'ldskcf_member_details.xlsx',
